...
\begin{definition}[$\sigma$-алгебра]
    ...
    $U$~--- $\sigma$-алгебра подмножеств $X$ $\Leftrightarrow$
    \begin{enumerate}
        \item $\varnothing \in U$
        \item $\forall A \in U (X \setminus A) \in U$
        \item $\seq{A_n}^\infty_{n=1}$~--- последовательность в $U \implies \bigcup^\infty_{n=1} A_n \in U$
    \end{enumerate}
\end{definition}
\begin{definition}[Мера]
    $X$~--- множество, $U$~--- $\sigma$-алгебра подмножеств $X$, $\mu : U \rightarrow [0, +\infty]$

    $\mu$~--- мера $\Leftrightarrow$
    \begin{enumerate}
        \item $\mu(\varnothing) = 0$
        \item $(\seq{A_n}^\infty_{n=1}$~--- последовательность в $U \land A = \bigcup^\infty_{n=1} A_n \land \forall n \in \bN \forall m \in \bN (n \ne m \implies A_n \cap A_m = \varnothing)) \implies \mu(A) = \sum^\infty_{n=1} \mu(A_n)$~--- счётная аддитивность.
    \end{enumerate}
    
    $(X, U, \mu)$ называется тогда пространством с мерой. 
\end{definition}

\begin{definition}[Полная мера]
    $\mu$~--- полная мера $\Leftrightarrow$

    $((A \in U \land \mu(A) = 0) \implies \forall B (B \subseteq A \implies B \in U))$.
\end{definition}

\begin{remark}
    Для $B$ из определения $\mu(B) = 0$.
\end{remark}

\begin{definition}[$\sigma$-конечная мера]
    $\mu$~--- $\sigma$-конечна $\Leftrightarrow$

    $\exists \seq{X_j}_{j=1}^\infty$~--- последовательность в $U (X = \bigcup^\infty_{j=1} X_j \land \forall j \in \bN \mu(X_j) < +\infty$
\end{definition}

Далее предполагаем: 
\begin{enumerate}
    \item $\mu$~--- полная мера
    \item $\mu$~--- $\sigma$-конечна
\end{enumerate}
...
\begin{definition}[Измеримая функция]
    $f: X \rightarrow \overline{\bR}$. $f$~--- измерима $\Leftrightarrow$
    \[ \forall c \in \bR \underbrace{\{x \in X | c < f(x) \}}_{\text{измеримое множество}} \in U \]

    \[ f: X \rightarrow \bC \Rightarrow \exists u \exists v (u: X \rightarrow \bR \land v: X \rightarrow \bR \land \forall x \in X f(x) = u(x) + iv(x)) \]
    $f$~--- измерима $\Leftrightarrow u,v$~--- измеримы
\end{definition}
...
\begin{multline*}
    S \coloneqq \{ g: X \rightarrow \bC | \exists n \in \bN \exists \seq{c_k}_{k=1}^n\text{~--- конечная последовательность в $\bC$}\\
    \exists \seq{e_k}_{k=1}^n\text{~--- конечная последовательность в $U$} g(x) = \sum^n_{k=1} c_k \chi_{e_k}(x) \}
\end{multline*}
\begin{definition}
$g \in S \Rightarrow$

\[\int_X g d\mu \coloneqq \sum^n_{k=1} c_k \mu e_k \]
\end{definition}

\begin{definition}[Интеграл неотрицательной функции]
    $f: X \rightarrow \bR, \forall x \in X f(x) \geq 0, H \coloneqq \{h \in S | \forall x \in X 0 \leq h(x) \leq f(x)\} \Rightarrow$
    \[ \int_X f d\mu \coloneqq \sup_{g \in H} \int_X g d\mu \]
\end{definition}

\begin{definition}
    $f: X \rightarrow \bR \Rightarrow$
    \[f_+(x) = \max{(f(x), 0)}\]
    \[f_-(x) = \max{(-f(x), 0)}\]
\end{definition}

Очевидно, что $f = f_+ - f_-, |f| = f_+ - f_-$.

\begin{definition}[Интеграл измеримой функции]
    $f: X \rightarrow \bR, \min{(\int_X f_+ d\mu, \int_X f_- d\mu)} < +\infty$, $f$~--- измерима $\Rightarrow$
\[ \int_X f d\mu \coloneqq \int_X f_+ d\mu - \int_X f_- d\mu \]
\end{definition}
\begin{definition}[Интеграл комплексной измеримой функции]
$f: X \rightarrow \bC, u: X \rightarrow \bR, v: X \rightarrow \bR, f = u + iv, f$~--- измерима $\Rightarrow$
\[ \int_X f d\mu \coloneqq \int_X u d\mu + i \int_X v d\mu \]
\end{definition}
\begin{definition}[Множество суммируемых функций]
    \[ L(X, \mu) \coloneqq \left\{ f: X \rightarrow \bR | \int_X |f| d\mu < + \infty \right\} \]
    $L(X, \mu)$~--- множество суммируемых функций
\end{definition}

% что-то не то
Прежде чем двигаться дальше, приведем примеры других мер (кроме меры Лебега)
\begin{example}
    $E \subseteq \bR^n$, $E$~--- измерима по Лебегу, $\lambda$~--- мера Лебега, $\forall x \in E w(x) \geq 0$, $w$~--- измерима по Лебегу.
    
    $e \subseteq E, e$~--- измеримо по Лебегу.

    $ \mu e \coloneqq \int_e w d \lambda$. Здесь $w(x)$ называется плотностью меры $\mu$, $w(x)$~--- её весом.\footnote{Упс, я здесь не разобрался.}
\end{example}
Вторая мера в каком-то смысле противоположная. Она сосредоточена на наборе точек и называется дискретной.
\begin{example}
    $X$~--- непустое множество, $a \in X, e \subseteq X, e$~--- измеримо
    \[ \delta_a(e) \coloneqq \begin{cases}
        1, a \in e \\
        0, a \notin e 
    \end{cases} \] 
\end{example}
...
\begin{example}[Дискретная мера]
    ...
    \[ \mu e \coloneqq \sum^\infty_{\gamma = 1} h_j \delta_{a_j}(e), e \subseteq X \quad \mu E = \sum_{\{ j: a_j \in E \}} h_j \]
\end{example}
% конец
...
\begin{theorem}[Неравенство Юнга]
    $p > 1, q > 1, \frac{1}{p} + \frac{1}{q} = 1$ ($q$~--- сопряженный показатель), $a \geq 0, b \geq 0 \Rightarrow$
    \[ ab \leq \frac{a^p}{p} + \frac{b^q}{q} \]
\end{theorem}

\begin{proof}
    Пусть $b$~--- фиксировано, $\varphi(x) \coloneqq \frac{x^p}{p} - xb, x \in [0, + \infty)$. Хотим найти $\min_{x \in [0, +\infty)} \varphi(x)$. Для этого посмотрим, где производная обращается в 0.

    $\varphi^\prime(x) = x^{p-1} - b \Rightarrow \varphi^\prime(x_0) = 0 \iff x_0 = b^{\frac{1}{p-1}}, \forall x \in [0, b^{\frac{1}{p-1}}) \varphi^\prime(x) < 0, \forall x \in (b^{\frac{1}{p-1}}, +\infty) \varphi^\prime(x) > 0 \Rightarrow \forall x \in [0, +\infty) (x \ne x_0 \implies \varphi(x) > \varphi(x_0))$.
    ...
    \begin{gather*}
        ...
        \forall x \in [0, + \infty) \varphi(x) \geq - \frac{b^q}{q}
    \end{gather*}
    В частности $\varphi(a) = \frac{a^p}{p} - ab \geq -\frac{b^q}{q} \Rightarrow ab \leq \frac{a^p}{p} + \frac{b^q}{q}$.
\end{proof}
...
\begin{theorem}[Неравенство Гёльдера]
    $(X, U, \mu)$~--- пространство с мерой. $f, g$~--- измеримые, $p > 1, q > 1, \frac{1}{p} + \frac{1}{q} = 1 \Rightarrow$
    \[ \int_X |fg| d\mu \leq \left( \int_X |f|^p d\mu \right)^{\frac{1}{p}} \cdot \left( \int_X |g|^q d\mu \right)^{\frac{1}{q}} \tag{*} \]
\end{theorem}
Если $p=q=2$, то это <<Неравенство Коши-Буняковского-Шварца>>, или на молодёжном математическом сленге неравенство КБШ.

\begin{proof}
Для начала отбросим какие-то простые случаи. \\
$A \coloneqq \left( \int_X |f|^p d \mu \right)^{\frac{1}{p}}, B \coloneqq \left( \int_X |g|^q d \mu \right)^{\frac{1}{q}}$.

$A = 0 \Leftrightarrow |f| = 0$ почти всюду по $\mu$ $\Leftrightarrow f(x) = 0$ почти всюду по $\mu$ (то есть $\mu \{x \in X | f(x) \ne 0 \} = 0)$
На всякий случай поясним, почему функция равна 0 почти всюду по мере $\mu$.
    \[ e \coloneqq \{x \in X | f(x) = 0 \}\]
    \[ m \in \bN, e_m \coloneqq \{x \in X | |f(x)| > \frac{1}{m} \} \]
    \[E \coloneqq \bigcup^\infty_{m=1} e_m\]
\[ \forall m \in \bN 0 = \int_X |f| d \mu \geq \int_{e_m} |f| d\mu \geq \frac{1}{m} \mu e_m \geq 0 \Rightarrow \forall m \in \bN \mu e_m = 0 \Rightarrow \mu E = 0 \]

\[ \Rightarrow f(x) \cdot g(x) = 0 \text { п.в. } \quad 0 \leq 0 \tag{*} \]
Если $A = +\infty$, то (*) % что-то не то
\[ \text{ пусть } 0 < A < +\infty, 0 < R < +\infty \]
Неравенство Гёльдера однородное, то есть если мы $f$ умножим на константу, то левая и правая часть умножится на неё же, аналогично с $g$. Иногда
бывает удобно ввести нормировку.
\[ f_1(x) \coloneqq \frac{f(x)}{A}, g_1(x) \coloneqq \frac{g(x)}{B}, \int_X |f_1(x)|^p d\mu = \frac{A^p}{A^p} = 1, \int_X |g_1(x)|^q d \mu = 1 \]

По неравенству Юнга
\begin{multline*}
    \forall x \in X |f_1(x)| \cdot |g_1(x)| \leq \frac{|f_1(x)|^p}{p} + \frac{|g_1(x)|^q}{q} \\
...
\begin{proof}
    ...
    $a \in \bR, b \in \bR \Rightarrow |a+b| \leq |a| + |b| \leq 2 \max(|a|, |b|) \Rightarrow |a+b|^p \leq 2^p \max(|a|^p, |b|^p) \leq 2^p(|a|^p + |b|^p) \Rightarrow$
    \[ \forall x \in X |f(x) + g(x) |^p \leq 2^p (|f(x)|^p + |g(x)|^p) \text{ проинтегрируем по } X \]
    ...
    \[ \int_X |f| \cdot |f+g|^{p-1} d\mu \stackrel{\text{ н. Гельдера}}{\leq} \underbrace{\left( \int_X |f|^p d\mu \right)^{\frac{1}{p}}}_{A} \cdot \left( \int_X |f+g|^p d\mu \right)^{\frac{p-1}{p}} = AC^{p-1} \]
    \begin{gather*}
        \int_X |g| \cdot |f+g|^{p-1} d \mu \stackrel{\text{аналогично}}{\leq} BC^{p-1} \Rightarrow \\
        C^p \leq (A+B) C^{p-1}, \quad 0 < C < + \infty \Rightarrow \\
        C \leq A + B \text{ (это (*))}
    \end{gather*}
\end{proof}
...
\[f \in \calL^p(X, \mu) \Rightarrow ||f||_p \coloneqq \left( \int_X |f(x)|^p d \mu \right)^{\frac{1}{p}} \]

Проверим, что $||f||_p$~--- это полунорма на $\calL^p(X,\mu)$. $c \in \bR$ (или $\bC$). $||cf||_p = |c| ||f||_p$

\[ ||f+g||_p \leq ||f||_p + ||g||_p \text{~--- неравенство Минковского} \]
$||f||_p = 0 \iff \int_X |f(x)|^p d\mu = 0 \iff f(x) = 0$ почти всюду по мере $\mu$ на $X$.

\begin{example}
    $L([0,1], \lambda), \lambda$~--- мера Лебега на $[0,1]$. \\
...
$N \coloneqq \{ f | f \text{~--- измерима} \, \land \, f(x) = 0 \text{ почти всюду на } X \text { по } \mu \} \Rightarrow \forall p \in \bN (||f||_p = 0 \iff f \in N)$.

$N$~--- подпространство в $\calL^p$. Определим факторпространство.

\begin{definition}
    $L^p \coloneqq \calL^p / N$.

    $g \in \calL^p, f \in \calL^p \Rightarrow$

    $f \sim g \Leftrightarrow f - g \in N \Leftrightarrow f(x) = g(x)$ почти всюду по $\mu$.
\end{definition}

$\overline{f} = \{g | f \sim g \}$~--- класс эквивалентности, 

$||\overline{f}||_p \coloneqq ||f||$, то есть можно взять любую функцию из класса эквивалентности.

\[ \forall \overline{f} \in \calL^p (||\overline{f}||_p = 0 \iff \int_X |f|^p d\mu = 0 \iff f \in N \iff \overline{f} = N = \overline{0}) \Rightarrow \]
$||\overline{f}||_p $~--- норма на $L^p$.

Говорят, <<возьмём функцию $f$ из $L^p$>>, но имеют в виду, что возьмут класс экивалентности, а из него возьмут функцию
...
\begin{definition}[$\calL^\infty(X, \mu)$]
    \[ \calL^\infty(X, \mu) \coloneqq \{f | \exists c > 0 \mu(\{ x \in X | \, |f(x)| > c \}) = 0\} \]

    Иными словами, $f \in \calL^\infty(X, \mu) \Leftrightarrow$
    \[\exists c > 0 |f(x)| \leq c \text{ почти всюду на } X \text{ по } \mu \, (\mu \{ x \in X | \, |f(x)| > c\} = 0)\]
\end{definition}

Возьмём точную нижнюю грань этой константы.
\begin{definition}[Существенный супремум]
    $\esssup_X f = \inf \{ c \geq 0 | \mu \{ x \in X | \, ||f(x)|| > c \}\}$
\end{definition}

Дальше $||f||_\infty \coloneqq \esssup_X f$.
...
\begin{proof}
    $e \coloneqq \{ x \in X | \, |f(x)| > ||f||_\infty \}$.
    $m \in \bN, e_m \coloneqq \{ x \in X | \, |f(x)| > ||f||_\infty + \frac{1}{m} \} \Rightarrow \mu e_m = 0$ по определеннию  $\esssup_X f$ $\Rightarrow e = \cup^\infty_{m=1} e_m \Rightarrow \mu e = 0$ 
\end{proof}
...
\begin{gather*}
    ...
    f \in \calL^\infty, g \in \calL^\infty, x \in X \Rightarrow |f(x) + g(x)| \leq |f(x)| + |g(x)| \leq ||f||_\infty + ||g||_\infty \text{ для п.в. } x \text { на } X \\
    \Rightarrow ||f+g||_\infty < ||f||_\infty + ||g||_\infty
\end{gather*}
$N \coloneqq \{ f \in \calL^\infty | f \text{~--- измерима} \land f(x) = 0 \text{ п.в. на } X \}$

$||f||_\infty = 0 \Leftrightarrow \mu \{ x \in X | \, |f(x)| > 0 \} = 0 \Leftrightarrow f(x) = 0$ п.в. на $X$ $\Leftrightarrow f \in N$
...
\end{proof}
    ...
    \end{definition}
$F \subseteq l^p \quad 1 \leq p \leq + \infty$.
