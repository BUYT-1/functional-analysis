...
Предположения: 
\begin{enumerate}
    \item $\mu$~--- полная мера, то есть $A \in U, \mu(A) = 0 \Rightarrow (\forall B \subseteq A \Rightarrow B \in U, (\Rightarrow \mu B) =0)$
...
\begin{example}
    $E \subseteq \bR^n$, $E$~--- измерима по Лебегу, $\lambda$~--- мера Лебега, $w(x) \geq 0, x \in E$, $w$~--- измерима по Лебегу. \\
...
\begin{example}
    $X$~--- множество ($X \ne \varnothing$), $a \in X$
    \[ \sigma_n, e \subseteq X, \sigma_a(e) = \begin{cases}
        1, a \in E \\
        0, a \notin e 
    \end{cases} \] 
    $\forall e, e \subseteq  X, e$~--- измеримо
\end{example}
...
\begin{example}[Дискретная мера]
    ...
    \[ \mu - \sum^\infty_{\gamma = 1} h_j \delta_{a_j}, e \subseteq X \quad \mu E = \sum_{\{ j: a_j \in E \}} h_j \]
\end{example}
