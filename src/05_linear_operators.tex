...
\begin{example}[оператор вложения]
    $l^1 \subseteq l^2$, $x = \seq{x_n}^\infty_{n=1}, \sum^\infty_{n=1} |x_n| < + \infty, x \in l^1 \Rightarrow \sum^\infty_{n=1} |x_n|^2 < + \infty \Rightarrow x \in l^2$
    \begin{gather*} 
...
\begin{definition}[Выпуклое множество]
    $B \subseteq X, X$ ~--- линейное пространство. $B$ ~--- \textbf{ выпуклое }, если 
...
\begin{theorem}[простейшие свойства линейного оператора]
    ...
    \begin{enumerate}
        \item $L \subseteq X, L$ ~--- подпространство в $X$ $\Rightarrow A(L)$ ~--- подпространство в $Y$ (образ подпространства ~--- подпространство)
        \item $M \subseteq Y, M$ ~--- подпространство в $Y \Rightarrow$ $\underbrace{A^{-1}(M)}_{\text{прообраз}}$ ~--- подпространство в $X$
        \item $B \subseteq X, B$ ~--- выпуклое $\Rightarrow A(B)$ ~--- выпуклое в $Y$
        \item $C \subseteq Y, C$ ~--- выпуклое $\Rightarrow A^{-1}(C)$ ~--- выпуклое в $X$
...
\begin{definition}[Огранисченный оператор]
    $(X, || \cdot||)$, $(Y, || \cdot ||), A \in \Lin(X,Y) $. $A$ ~--- \textbf{ ограниченный}, если $\forall C \subseteq X, C$ ~--- ограниченное $\Rightarrow A(C)$ ~--- ограниченное 
    в $Y$.
\end{definition}
...
\begin{proof}[$3 \Rightarrow 4$]
    $B \subseteq X$, $B$ ~--- ограниченное, то есть $\exists M > 0 : (\forall x \in B \land ||x|| < M) \stackrel{3}{\Rightarrow} ||Ax|| \leq C||x|| \leq CM \, \forall x \in B \Rightarrow 
    \{ A(B) \}$ ~--- ограниченное.
\end{proof}
$4 \Rightarrow 5$ очевидно ($B_r(0)$ ~--- ограниченное)
\begin{proof}[$5 \Rightarrow 1$]
    $\exists R > 0 \, A(B^x_r(0)) \subseteq B^y_R(0)$
    \begin{gather*}
        ||x|| < r \Rightarrow ||Ax|| < R \\
...
\begin{example}
    ...
    \begin{gather*}
        X \subseteq Y, X \text{ ~--- подпространство } Y, \text{ то есть } \\
...
\begin{definition}
    ...
    По-другому можно сказать, что топологии, которые задают эти  нормы, одинаковые: $\Leftrightarrow G \subseteq X, G$ --- открытое в $(X, \norm{\cdot}_1) \Leftrightarrow G$ --- открытое в $(X, \norm{\cdot}_2)$
\end{definition}
...
\begin{corollary}
    ...
    \begin{enumerate}
        \item $X$ --- банахово 
        \item $K \subseteq X, K$ --- относительно компактно $\Leftrightarrow K$ --- ограничено
        \item $K \subseteq X, K$ --- компакт $\Leftrightarrow K$ --- ограничено и замкнуто  
    \end{enumerate}
\end{corollary}
...
\begin{definition}
    $(X,\rho)$ --- метрическое, $Y \subseteq X, x_0 \in X, \rho(x_0,Y) = \inf_{y \in Y} \rho(x_0, y)$. Если $\exists y_0 \in Y$ т.ч.$ \rho(x_0, Y) = \rho(x_0,y_0)$, то
...
\begin{theorem}
    $(X, \norm{\cdot})$ --- нормированное, $L \subseteq X$. $L$ --- подпространство (в алгебраическом смысле), $\dim L = n < + \infty \Rightarrow$
    \begin{enumerate}
        \item $L$ --- замкнутое \\
...
\begin{theorem}[о продолжении линейного оператора]
    $(X, \norm{\cdot})$ --- нормированное, $(Y, \norm{\cdot})$ --- банахово, $L \subseteq X, L$ --- подпространство в алгебраическом смысле
